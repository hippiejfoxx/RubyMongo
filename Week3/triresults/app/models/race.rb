class Race
  ...
  DEFAULT_EVENTS.keys.each do |name|
    ...
    ["order","distance", "units"].each do |x|
      if DEFAULT_EVENTS["#{name}"][x.to_sym]
        define_method("#{name}_#{x}") do
          event = self.send("#{name}").send("#{x}")
        end
        define_method("#{name}_#{x}=") do |value|
          event = self.send("#{name}").send("#{x}=", value)
        end
      end
    end
  end
